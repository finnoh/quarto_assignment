...
\verb+logwage+ is only observed when a worker earns a wage.

Let $Z_{i}^{'}$, $\gamma$, and $V_{i}$ denote some exclusion restriction, the corresponding coefficient, and error term for individual $i$, respectively. Then, we have the selection equation
\begin{align*}
    I_{i}^{*} = Z_{i}^{'} \gamma + V_{i},
\end{align*}
where $I_{i}$ takes value $1$ if the wage for individual $i$ $I_{i}^{*} > 0$, and value $0$ otherwise. In addition, let $Y_{i}^{*}$ denote the latent variable \verb+logwage+, $X_{i}^{'}$ the regressor(s), $\beta$ the coefficient (vector) and $U_{i}$ the error term. The second regression equation is
\begin{align*}
    Y_{i}^{*} = X_{i}^{'}\beta + U_{i}.
\end{align*}
